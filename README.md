...
1. 在「走進現場」選 Fake Case 或 Real Case（Gregory Bright & Earl Truvia），直接探索。
2. 按「進入現場」直接使用 i.MX SLAM／NPU 手勢控制；畫面顯示相機預覽與追蹤狀態。
3. 使用姿態防抖、畫面平滑與位移倍率調整探索；按「重設位置與正前方」重新建立原點。
...
在世界資料庫按「進入現場」，直接開啟目前選取世界的 SLAM／NPU 手勢控制畫面，包含相機預覽、姿態追蹤、手勢偵測與控制設定。按「返回現場資料庫」可選擇其他世界。
