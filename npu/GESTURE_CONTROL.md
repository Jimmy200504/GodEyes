...
揮手時只旋轉，不同時前後移動；停止揮動或暫時漏偵測後，方向最多保留到最後一次有效動作的一秒期限；握拳立即取消。揮完先握拳，才能再用掌心正反控制前後。食指高信心辨識一次即觸發，不再要求連續兩次；分類前會將骨架方向轉正，控制仍使用原始影像座標。食指看的是指向，與手放在畫面的哪個位置無關。張掌前後移動需先穩定保持約半秒。

偵測到手（包含拳頭）或正在延續動作時，暫停 SLAM 對視角的更新，只疊加手勢位移與旋轉。背景 SLAM 仍追蹤；延續動作結束後，連續至少 350 ms 的新鮮結果確認沒有手，再等待新的 SLAM 姿態，以當前視角接回，避免跳回原位置。資料過期或斷線不當成手已離開。
...
有效方向保留最多 1 秒（從來源影格時間計），新的有效方向會立即取代；空結果及 WebSocket 輪詢不會延長期限。握拳、校正或相機錯誤取消延續；影格過期、失焦或 Socket 中斷仍立即停止前端控制。右上角保留真實的偵測標籤，延續期間另外顯示動作與剩餘時間。
