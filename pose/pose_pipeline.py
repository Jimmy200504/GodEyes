...
class PosePipeline:
    def __init__(self, marker_m=.055, calibration=None, approximate=False):
        ...
        self.map_id = f'screen-B:0:{marker_m}:{identity}'
            ...
            if values.count(0) != 1:
                ...
                if np.min(np.linalg.norm(pixels-np.roll(pixels,1,axis=0),axis=1)) < 20:
                    ...
                    if self.calibration:
                        ...
                        # Deliberate rough demo assumption, NOT a C270 calibration/FOV measurement.
                        width,height = size
