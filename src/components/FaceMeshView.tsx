...
import {
  HeadPose,
  HeadNavigationTracker,
  DEFAULT_ROTATION_GAINS,
} from "../utils/headPose";
...
export default function FaceMeshView({
  ...
  const [rotationGains, setRotationGains] = useState({
    ...DEFAULT_ROTATION_GAINS,
  });
    ...
    const start = async () => {
      try {
        ...
        detector = await FaceLandmarker.createFromOptions(files, {
          baseOptions: {
            modelAssetPath: "/tracking/face_landmarker.task",
          },
          runningMode: "VIDEO",
          numFaces: 1,
        ...
        const process = () => {
          ...
          try {
            ...
            if (
              ...
              const pose = matrix
                ? navigationTracker.current.update(matrix.data)
                : null;
            ...
            setLastError(
              error instanceof Error ? error.message : "頭部追蹤失敗",
            );
            setTracking(false);
        ...
        if (!cancelled) {
          setLastError(
            error instanceof Error ? error.message : "頭部追蹤載入失敗",
          );
          setIsLoading(false);
        }
      }
      ...
      {controlsContainer &&
        createPortal(
          <div className="w-64 rounded-lg bg-black/70 p-3 text-xs text-white backdrop-blur-sm">
            ...
            <button
              type="button"
              disabled={paused || !tracking || isLoading}
              className="mt-3 w-full rounded bg-white/20 px-2 py-1 disabled:opacity-40"
              onClick={() => navigationTracker.current.reset()}
            >
              重設位置
            </button>
          </div>,
        ...
        <div className="absolute bottom-2 inset-x-2 rounded bg-black/70 p-2 text-xs text-white">
          ...
          {lastError && (
            <button
              ...
              onClick={() => {
                if (cameraReady) setAttempt((value) => value + 1);
