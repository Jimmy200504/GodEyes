...
export default function App() {
  ...
  return <main className="h-screen w-screen relative bg-black">
    <ThreeView headPose={pose && pose.position ? { ...pose, position: { x: pose.position.x * gain, y: pose.position.y * gain, z: pose.position.z * gain } } : pose} renderSmoothing={renderSmoothing} posePrediction={posePrediction} allowCoast={allowCoast} poseEpoch={poseEpoch} />
    <CameraPreview />
    <div className="absolute bottom-4 left-4 z-20 rounded-lg bg-black/80 p-4 text-white space-y-2">
      <h1 className="font-bold">GodEyes · 無標記 SLAM 實驗版</h1>
