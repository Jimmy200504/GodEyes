...
npm run build  # Rebuild the current advanced SLAM UI; never reuse a stale dist.
"$SLAM_PYTHON" - <<'PY'
import socket
for port in (18781, 8865, 8866, 8867, 5182):
...
ssh -N -o ExitOnForwardFailure=yes -o ServerAliveInterval=15 -o ServerAliveCountMax=3 \
  -L 127.0.0.1:18781:127.0.0.1:8781 "${SLAM_BOARD:-root@100.86.170.121}" &
tunnel_pid=$!
...
PY
node node_modules/vite/bin/vite.js preview --host 127.0.0.1 --port 5182 --strictPort &
web_pid=$!
